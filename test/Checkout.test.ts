...
test('Item não pode ser repetido', async function() {
  ...
  expect(()=> checkout.execute(input)).rejects.toThrow(new Error('Repeated item'));
});
...
test('Deve criar um pedido com 1 produto em dólar usando um stub', async function() {
  ...
  const input = {
    cpf: '407.302.170-27',
    items: [
      { idProduct: 9, quantity: 1 }
    ]
  }
  const output = await checkout.execute(input);
  expect(output.total).toBe(3000);
  stubCurrencyGateway.restore(); // para não interferir nos próximos testes
...
test('Deve criar um pedido com 1 produto em dólar usando um mock', async function() {
  ...
  mockCurrencyGateway.restore(); // para não interferir nos próximos testes  
});
...
test('Deve criar um pedido com 1 produto em dólar usando um fake', async function() {
  ...
  const productRepository: ProductRepository = {
    async getProduct(idProduct: number): Promise<any> {
      return { idProduct: 5, description: 'A', price: 1000, width: 10, height: 10, length: 10, weight: 10, currency: 'USD' }
    }
  }
