...
test('Não pode adicionar item repetido', function (){
  const uuid = crypto.randomUUID();
  const cpf = '407.302.170-27';
  const order = new Order(uuid, cpf);
  order.addItem(new Product(1, 'A', 1000, 100, 30, 10, 3, 'BRL'), 1);
  expect(() => order.addItem(new Product(1, 'A', 1000, 100, 30, 10, 3, 'BRL'), 1)).toThrow(new Error('Repeated item'));
});
...
test('Deve criar um pedido com 3 itens, sendo 1 em dólar', function (){
  const uuid = crypto.randomUUID();
  const cpf = '407.302.170-27';
